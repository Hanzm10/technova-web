...
import { ShoppingBag, ArrowLeft, Sun } from 'lucide-react';
        ...
        const fetchProducts = async () => {
            try {
                ...
                const { data, error } = await supabase
                    .from('products')
                    .select('*')
                    .or('name.ilike.%Sonic Earbuds Pro%,name.ilike.%Aeroweave Runners%,name.ilike.%Nova Smart Watch%');

                if (error) throw error;

                // Sort to ensure the order requested if possible
                const orderedProducts = [
                    data?.find(p => p.name.toLowerCase().includes('sonic earbuds pro')),
                    data?.find(p => p.name.toLowerCase().includes('aeroweave runners')),
                    data?.find(p => p.name.toLowerCase().includes('nova smart watch'))
                ].filter(Boolean) as Product[];

                setProducts(orderedProducts.length > 0 ? orderedProducts : (data || []));
            } catch (error) {
                console.error('Error fetching summer collection products:', error);
            } finally {
                setIsLoading(false);
            }
        };
                    ...
                    <div className="container mx-auto px-6 relative z-30">
                        ...
                        >
                            <div className="flex items-center space-x-4 mb-4">
                                ...
                                <span className="text-white/80 font-mono text-sm tracking-[0.3em] uppercase">SUMMER ESSENTIALS 2026</span>
                            </div>
                            <h1 className="text-5xl sm:text-7xl md:text-9xl font-bold text-white tracking-tighter leading-[0.85] mb-6">
                                WARM VIBES, <br /> COOL <span className="text-white/40 italic font-serif">design</span>
                            </h1>
                            <p className="text-white/70 max-w-xl text-lg md:text-xl font-medium leading-relaxed">
                                Engineered for the Southeast Asian summer heat. Light fabrics, portable tech, and sun-ready innovations for your lifestyle.
                            </p>
                        </motion.div>
                    </div>
                </section>
                    ...
                    <div className="container mx-auto">
                        <div className="bg-white/40 backdrop-blur-xl border border-white/40 rounded-[3rem] p-12 shadow-2xl overflow-hidden">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 px-4">
                                <div>
                                    <h2 className="text-4xl md:text-5xl font-bold text-slate-900 tracking-tighter">HOT PICKS</h2>
                                    <p className="text-slate-600 mt-2">Lightweight gear for heavy-duty summer days.</p>
                                </div>
                                <div className="mt-8 md:mt-0 text-right">
                                    <span className="text-slate-400 font-mono text-sm tracking-widest uppercase">33°C / UV INDEX 11</span>
                                </div>
                            </div>
